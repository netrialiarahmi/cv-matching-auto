#!/usr/bin/env python3
"""
Kalibrr Export — Dashboard (Active) Positions Only
...
async def main():
    ...
    if not positions:
        print("\n✅ Tidak ada posisi aktif (dashboard) dengan Job ID. Selesai.")
        return

    print(f"\n📋 Ditemukan {len(positions)} posisi aktif (dashboard):")
    for p in positions:
    ...
    if not positions_to_export:
        print("\n✅ Semua posisi aktif sudah memiliki File Storage. Tidak ada yang perlu di-export.")
        return
    ...
    print(f"\n✅ Selesai. {len(export_results)} posisi aktif berhasil di-export.")
